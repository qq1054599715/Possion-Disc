...
public class PossionDisc2
{
    ...
    private Func<Vector3, bool> otherCondition;
    
    public PossionDisc2(float width, float height, float minDistance, int k = 30,Func<Vector3 ,bool > otherCondition = null  )
    {
    ...
    private bool IsFit(int idx, int idy, Vector3 newPosition)
    {
        ...
        bool otherConditionOk = true;
        if (this.otherCondition != null)
        {
            otherConditionOk = this.otherCondition(newPosition);
        }

        if (otherConditionOk == false)
        {
            fit = false;
            return fit;
        }
